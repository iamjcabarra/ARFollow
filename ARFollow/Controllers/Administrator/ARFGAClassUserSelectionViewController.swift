...
class ARFGAClassUserSelectionViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, NSFetchedResultsControllerDelegate, UISearchBarDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.navigationBar.topItem?.title = self.isForCreator ? "Select Creator" : "Select Player"
        ...
        self.searchBar.placeholder = "Search \(self.isForCreator ? "Creator" : "Player")"
    ...
    @objc fileprivate func selectButtonAction(_ sender: UIButton) {
        ...
        if self.selectedUsers == nil || self.selectedUsers?.count == 0 {
            let title = self.isForCreator ? "Select Creator" : "Select Player"
            let subtitle = "Please select \(self.isForCreator ? "creator" : "at least one player") to continue."
    ...
    fileprivate func shouldShowEmptyPlaceholderView(_ show: Bool) {
        if (show) {
            let userType = self.isForCreator ? "creator" : "player"
