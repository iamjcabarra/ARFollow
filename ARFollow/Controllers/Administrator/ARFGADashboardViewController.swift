...
class ARFGADashboardViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, NSFetchedResultsControllerDelegate, UIPopoverPresentationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        let action = #selector(self.refreshActivityList)
        ...
        self.emptyPlaceholderLabel.text = "No recent activity."
    }
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        /// Request for list of activities
        self.reloadActivityList()
    }
    ...
    @objc fileprivate func refreshActivityList() {
        self.arfDataManager.requestRetrieveActivities { (result) in
            let status = result!["status"] as! Int
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: ARFConstants.cellIdentifier.ACTIVITY, for: indexPath) as! ARFGAActivityTableViewCell
        self.configureCell(cell, atIndexPath: indexPath)
        return cell
    }
    
    func configureCell(_ cell: ARFGAActivityTableViewCell, atIndexPath indexPath: IndexPath) {
        let activityObject = fetchedResultsController.object(at: indexPath) as! Activity
        let dateString = self.arfDataManager.string(fromDate: activityObject.date ?? Date(), format: ARFConstants.timeFormat.CLIENT_LONG_DATE_ONLY)
        let timeString = self.arfDataManager.string(fromDate: activityObject.date ?? Date(), format: ARFConstants.timeFormat.CLIENT_LONG_NON_MILITARY_TIME_ONLY)
        let endString = activityObject.date == nil ? "." : "last \(dateString) at \(timeString)."
        
        cell.activityLabel.text = "User with id \"\(activityObject.userId)\" \(activityObject.activity!.lowercased()) via the \(activityObject.module!) module \(endString)"
        cell.contentView.backgroundColor = indexPath.row % 2 == 0 ? UIColor(hex: "EBEBF1") : UIColor(hex: "FFFFFF")
    }
    ...
    fileprivate var fetchedResultsController: NSFetchedResultsController<NSManagedObject> {
        ...
        let fetchRequest: NSFetchRequest<NSManagedObject> = NSFetchRequest<NSManagedObject>(entityName: ARFConstants.entity.ACTIVITY)
        ...
        case .update:
            if let indexPath = indexPath {
                if let cell = self.tableView.cellForRow(at: indexPath) {
                    self.configureCell(cell as! ARFGAActivityTableViewCell, atIndexPath: indexPath)
                }
            }
            break;
        case .move:
